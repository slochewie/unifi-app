...
const ARTWORK_SOURCES: Record<string, string> = {
  ...
  "u-lte-backup-pro": "https://cdn.ecomm.ui.com/products/6fa4d0a4-b362-4d52-8ac2-2a5e80c45854/32792c8d-7216-449f-ae1c-9b0ea75e4e6f.png",
...
function normalizeFirmwareVersion(version?: string | null) { return version?.split("+", 1)[0] ?? null }
function normalizeMacAddress(mac?: string | null) { if (!mac) return null; const compact = mac.replace(/[^0-9a-f]/gi, "").toLowerCase(); if (compact.length !== 12) return mac.toLowerCase(); return compact.match(/.{2}/g)?.join(":") ?? mac.toLowerCase() }
...
function connectorNetworkBase(hostId: string) { return `https://api.ui.com/v1/connector/consoles/${encodeURIComponent(hostId)}/proxy/network` }
...
function artworkUrl(device: LegacyDevice, cloudDevice?: SiteManagerDevice) { const key = artworkKey(device, cloudDevice); return key ? ARTWORK_SOURCES[key] ?? null : null }
function deviceCategory(type?: string, productLine?: string, model?: string) { if (model === "ULTEPUS") return "lte"; if (model === "USWDA23" || model === "USWDA25") return "ups"; const product = productLine?.toLowerCase() ?? ""; if (product.includes("gateway")) return "gateway"; if (product.includes("access point") || product.includes("wifi")) return "access-point"; if (product.includes("switch")) return "switch"; if (type === "ugw" || type === "uxg" || type === "udm") return "gateway"; if (type === "uap") return "access-point"; if (type === "usw") return "switch"; return "other" }
function mapDevice(device: LegacyDevice, cloudDevice?: SiteManagerDevice) { const category = deviceCategory(device.type, cloudDevice?.productLine, device.model); const isLteBackupPro = device.model === "ULTEPUS"; return { id: device._id ?? device.mac ?? `${device.model ?? "device"}-${device.ip ?? "unknown"}`, name: isLteBackupPro ? "LTE Backup Pro" : device.name?.trim() || MODEL_NAMES[device.model ?? ""] || device.model || "UniFi Device", model: isLteBackupPro ? "U-LTE-Backup Pro" : MODEL_NAMES[device.model ?? ""] ?? device.model ?? "Unknown model", category, ipAddress: category === "gateway" ? device.lan_ip ?? device.ip ?? null : device.ip ?? device.lan_ip ?? null, macAddress: device.mac ?? null, firmwareVersion: normalizeFirmwareVersion(device.displayable_version ?? device.version), firmwareStatus: device.upgradable === true ? "update-available" : device.upgradable === false ? "up-to-date" : "unknown", firmwareAvailableVersion: normalizeFirmwareVersion(cloudDevice?.updateAvailable), state: device.state ?? null, online: device.state === 1, adopted: device.adopted ?? null, uplink: device.uplink?.uplink_device_name ? { name: device.uplink.uplink_device_name, macAddress: device.uplink.uplink_device_mac ?? null, port: device.uplink.uplink_remote_port ?? null } : null, imageUrl: artworkUrl(device, cloudDevice) } }
function mapCloudKey(host: NonNullable<HostResponse["data"]>, cloudDevice?: SiteManagerDevice) { const state = host.reportedState; const hardware = state?.hardware; const mac = hardware?.mac ?? state?.mac ?? cloudDevice?.mac ?? null; const updateAvailable = state?.deviceState === "updateAvailable"; return { id: `console-${host.id ?? mac ?? "cloudkey"}`, name: "UCK G2 Plus", model: "CloudKey+", category: "console" as const, ipAddress: state?.ip ?? null, macAddress: normalizeMacAddress(mac), firmwareVersion: normalizeFirmwareVersion(hardware?.firmwareVersion ?? state?.version), firmwareStatus: updateAvailable ? ("update-available" as const) : state?.firmwareUpdate?.latestAvailableVersion ? ("up-to-date" as const) : ("unknown" as const), firmwareAvailableVersion: updateAvailable ? normalizeFirmwareVersion(state?.firmwareUpdate?.latestAvailableVersion ?? cloudDevice?.updateAvailable) : null, state: null, online: state?.state === "connected", adopted: null, uplink: null, imageUrl: ARTWORK_SOURCES["uck-g2-plus"] } }
function cloudDeviceFor(device: LegacyDevice, devices: SiteManagerDevice[]) { const mac = device.mac?.replace(/:/g, "").toLowerCase(); if (mac) { const byMac = devices.find((candidate) => candidate.mac?.replace(/:/g, "").toLowerCase() === mac); if (byMac) return byMac } return devices.find((candidate) => candidate.model === device.model) }
function consoleDeviceFor(devices: SiteManagerDevice[]) { return devices.find((device) => device.model?.toLowerCase().includes("uck")) }
async function loadCloudKey(site: SiteConfig, cloudDevices: SiteManagerDevice[], apiKey: string) { if (!site.cloudKeyHostId) return null; try { const response = await unifiFetch<HostResponse>(`https://api.ui.com/v1/hosts/${encodeURIComponent(site.cloudKeyHostId)}`, apiKey); return response.data ? mapCloudKey(response.data, consoleDeviceFor(cloudDevices)) : null } catch (error) { console.warn("Unable to load UniFi console inventory record", site.cloudKeyHostId, error); return null } }
async function loadSiteDevices(site: SiteConfig, cloudSites: SiteManagerSite[], cloudDeviceGroups: SiteManagerDeviceGroup[], apiKey: string) { const candidates = cloudSites.filter((candidate): candidate is SiteManagerSite & { hostId: string } => candidate.siteId === site.siteId && Boolean(candidate.hostId)); for (const candidate of candidates) { try { const cloudDevices = cloudDeviceGroups.find((group) => group.hostId === candidate.hostId)?.devices ?? []; const [response, cloudKey] = await Promise.all([unifiFetch<LegacyResponse<LegacyDevice>>(`${connectorNetworkBase(candidate.hostId)}/api/s/default/stat/device`, apiKey), loadCloudKey(site, cloudDevices, apiKey)]); return { id: site.siteId, name: site.name, available: true, devices: [...(response.data ?? []).map((device) => mapDevice(device, cloudDeviceFor(device, cloudDevices))), ...(cloudKey ? [cloudKey] : [])] } } catch (error) { console.warn("Skipping unavailable UniFi console", candidate.hostId, error) } } return { id: site.siteId, name: site.name, available: false, devices: [] } }

async function handleDevices(request: Request) {
  const authorization = await authorizeNetworkStatusSite(request)
  if ("response" in authorization) return authorization.response
  const site = SITE_CONFIG.find((candidate) => candidate.siteId === authorization.site.siteId)
  if (!site) return Response.json({ error: "UniFi site configuration is unavailable" }, { status: 404 })
  ...
  try {
    ...
    const result = await loadSiteDevices(site, cloudSites.data ?? [], cloudDevices.data ?? [], apiKey)
    return Response.json({ updatedAt: new Date().toISOString(), sites: [result] })
  } catch (error) {
...
export const Route = createFileRoute("/api/devices")({ server: { handlers: { GET: async ({ request }) => await handleDevices(request) } } })
