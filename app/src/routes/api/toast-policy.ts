...
function sourceFilterApplies(filter: TrafficFilter | undefined, sourceCidrs: Set<string>) {
  ...
  const listedSource = ipFilter.items.some(
    (item) => item.type === "SUBNET" && item.value && sourceCidrs.has(item.value),
  )
...
async function handleToastPolicy(request: Request) {
  ...
  if (!organizationName || !zoneId) {
    return Response.json({ error: "organizationName and zoneId are required" }, { status: 400 })
  }
  ...
  if (!siteConfig) {
    return Response.json({ error: `No UniFi site mapping exists for ${organizationName}` }, { status: 404 })
  }
  ...
  try {
    ...
    if (!selectedZone || !externalZone) {
      return Response.json({ error: "Selected or External UniFi firewall zone was not found" }, { status: 404 })
    }
    ...
    const networkDetails = await Promise.all(
      networkSummaries.map((network) =>
        unifiFetch<Network>(`${sitePath}/networks/${encodeURIComponent(network.id)}`, apiKey),
      ),
    )
    const sourceCidrs = new Set(
      networkDetails.flatMap((network) => {
    ...
    const outboundPolicies = policies
      .filter(
        (policy) =>
          policy.source?.zoneId === selectedZone.id &&
          policy.destination?.zoneId === externalZone.id &&
          sourceFilterApplies(policy.source.trafficFilter, sourceCidrs),
      )
      .sort((a, b) => (a.index ?? 0) - (b.index ?? 0))
    const returnPolicies = policies
      .filter(
        (policy) =>
    ...
    const restrictingOutboundPolicies = outboundPolicies.filter(
      (policy) => policy.action?.type === "BLOCK" && !isInvalidTrafficPolicy(policy),
    )
    const outboundAllowAll = outboundPolicies.some(
      (policy) =>
        policy.action?.type === "ALLOW" &&
        !policy.source?.trafficFilter &&
        !policy.destination?.trafficFilter &&
        !policy.ipProtocolScope?.protocolFilter,
    )
    const returnTrafficAllowed = returnPolicies.some(
      (policy) => policy.action?.type === "ALLOW" && policy.action.allowReturnTraffic === true,
    )
    ...
    return Response.json({
      ...
      evidence: {
        outboundPolicies: outboundPolicies.map((policy) => ({
          ...
          action: policy.action?.type ?? null,
        })),
...
export const Route = createFileRoute("/api/toast-policy")({
  server: {
    handlers: {
      GET: async ({ request }) => await handleToastPolicy(request),
    },
  },
})
