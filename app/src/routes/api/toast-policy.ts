...
type FirewallPolicy = { id: string; name: string; enabled?: boolean; index?: number; action?: { type?: string; allowReturnTraffic?: boolean }; source?: { zoneId?: string; trafficFilter?: TrafficFilter }; destination?: { zoneId?: string; trafficFilter?: TrafficFilter }; ipProtocolScope?: { ipVersion?: string; protocolFilter?: unknown } }
...
function connectorBase(hostId: string) { return `${connectorRoot(hostId)}/integration/v1` }
async function resolveNetworkHost(sites: SiteManagerSite[], siteConfig: SiteConfig, apiKey: string) { const candidates = sites.filter((site): site is SiteManagerSite & { hostId: string } => site.siteId === siteConfig.siteId && Boolean(site.hostId)); for (const candidate of candidates) { try { const localSites = await unifiFetch<Page<LocalSite>>(`${connectorBase(candidate.hostId)}/sites?offset=0&limit=100`, apiKey); const localSite = localSites.data?.[0]; if (localSite) return { hostId: candidate.hostId, localSite } } catch (error) { console.warn("Skipping unavailable UniFi console", candidate.hostId, error) } } return null }
function ipv4NetworkCidr(address: string, prefixLength: number) { const octets = address.split(".").map(Number); if (octets.length !== 4 || octets.some((octet) => !Number.isInteger(octet) || octet < 0 || octet > 255)) return null; const value = (((octets[0] << 24) >>> 0) + (octets[1] << 16) + (octets[2] << 8) + octets[3]) >>> 0; const mask = prefixLength === 0 ? 0 : (0xffffffff << (32 - prefixLength)) >>> 0; const network = (value & mask) >>> 0; const parts = [24, 16, 8, 0].map((shift) => (network >>> shift) & 0xff); return `${parts.join(".")}/${prefixLength}` }
function sourceFilterApplies(filter: TrafficFilter | undefined, sourceCidrs: Set<string>) { if (!filter) return true; const ipFilter = filter.ipAddressFilter; if (!ipFilter?.items?.length) return true; const listedSource = ipFilter.items.some((item) => item.type === "SUBNET" && item.value && sourceCidrs.has(item.value)); return ipFilter.matchOpposite ? !listedSource : listedSource }
function isInvalidTrafficPolicy(policy: FirewallPolicy) { return policy.name.toLowerCase() === "block invalid traffic" }
...
export const Route = createFileRoute("/api/toast-policy")({ server: { handlers: { GET: async ({ request }) => await handleToastPolicy(request) } } })
